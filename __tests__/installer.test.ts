...
describe('installer tests', () => {
  ...
  it('Install FORM 5.0.0', async () => {
    await installer.installForm('5.0.0')
    const formDir = path.join(toolDir, 'form', '5.0.0', os.arch())
  ...
  it('Install FORM 4.3.1', async () => {
    await installer.installForm('4.3.1')
    const formDir = path.join(toolDir, 'form', '4.3.1', os.arch())
    expect(fs.existsSync(`${formDir}.complete`)).toBe(true)
    if (process.platform === 'win32') {
      expect(fs.existsSync(path.join(formDir, 'form.exe'))).toBe(true)
    } else {
